...
import { getDate, getDateError, getNumberOfBikes } from "../../app/store/selectors";
...
const DateSelect = () => {
    ...
    const dateIsValid = (picker, newValue) => {
        if ((picker === 'from' && !!!date.to) || (picker === 'to' && !!!date.from)) return true

        //result será 1 cuando la primera fecha sea mayor a la segunda
        const result = picker === 'from' ? compareAsc(date.to, newValue) : compareAsc(newValue, date.from)
        return result === 1 ? true : false
    }

    const handleError = (reason, value) => {
        reason ?
            dispatch(setDateError(`Seleccione una fecha superior a ${format(date.from, "dd/MM/yyyy")} o modifique la fecha de inicio`))
        ...
        >
            <Stack direction="row" spacing={1}>
                ...
                <DatePicker
                    ...
                    // onChange={handleChange('to')}
                    renderInput={(params) => <TextField {...params} helperText={errorDate && errorDate} />}
